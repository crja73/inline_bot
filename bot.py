...
async def process_delete(message: types.Message, state: FSMContext):
    ...
    conn = sqlite3.connect('accounts.db')
    cur = conn.cursor()
    cur.execute(f'DELETE FROM users WHERE rowid = "{message.text}"')
    conn.commit()
    await state.finish()
...
async def ordering(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        if 'Обратно в меню' in ordr:
            ...
            full_order = 'Пользователь: {}, Срок: {}, Бюджет: {}, Ссылка на Lolz: {}, ТЗ:  '.format(name, data['date'], data['price'], data['lolz']) + data['order']
